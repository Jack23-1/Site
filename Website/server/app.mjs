...
export async function createApp({ db, origin, production = false, trustProxyHops = 0 }) {
  const app = express()
  ...
  const csrf = (req, res, next) => {
    ...
    if (supplied.length !== expected.length || !timingSafeEqual(Buffer.from(supplied), Buffer.from(expected))) return res.status(403).json({ message: 'Jeton de sécurité invalide. Rechargez la page.' })
    next()
  }
  ...
  app.get('/api/admin/content', async (req, res) => {
    ...
    const rows = await db.content.findMany({ where: { type }, orderBy: [{ updatedAt: 'desc' }, { id: 'desc' }] })
    res.json({ items: rows.map(toItem) })
  })
  app.post('/api/admin/content', async (req, res) => {
    const item = contentInput.parse(req.body)
    const row = await db.content.create({ data: { ...toData(item), publishedAt: item.status === 'published' ? new Date() : null } })
  ...
  app.put('/api/admin/content/:id', async (req, res) => {
    ...
    const row = await db.$transaction(async transaction => {
      ...
      const update = await transaction.content.updateMany({ where: { id, version }, data: {
        ...toData(item), version: { increment: 1 },
  ...
  app.get('/api/content', async (req, res) => {
    ...
    const rows = await db.content.findMany({ where: { type, status: 'published' },
      orderBy: [{ publishedAt: 'desc' }, { id: 'desc' }], take: limit + 1,
