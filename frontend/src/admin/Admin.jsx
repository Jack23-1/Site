  ...
  return <div className="admin-root admin-shell">
    ...
    <div className="admin-workspace">
      ...
      <main className="admin-main">
        <header className="admin-heading"><div><span className="admin-eyebrow">VOTRE ESPACE ÉDITORIAL</span><h1>{sections[section]}<span className="admin-heading-dot" aria-hidden="true">.</span></h1><p>{section === 'news' ? 'Faites vivre l’actualité de l’ONIP.' : section === 'documents' ? 'Rendez l’information utile accessible à tous.' : section === 'carousel' ? 'Choisissez les photos et les messages à la une.' : section === 'contacts' ? 'Gardez vos coordonnées à jour pour les visiteurs.' : 'Racontez nos actions en images.'}</p></div>{section !== 'contacts' && <button className="admin-primary" disabled={busy} onClick={() => { setEditor(emptyContent(section)); setLocale('fr'); setError(''); setNotice('') }}><AdminIcon name="plus" /> Nouveau contenu</button>}</header>
        {error && <div className="admin-alert" role="alert">{error}</div>}
        ...
        {section === 'contacts' ? <ContactEditor request={request} onBusyChange={setBusy} /> : editor ? <form className="admin-editor" onSubmit={save}>
          <div className="admin-editor-heading"><div><span className="admin-eyebrow">RÉDACTION & PUBLICATION</span><h2>{editor.id ? 'Modifier le contenu' : 'Nouveau contenu'}</h2></div><button type="button" disabled={busy} onClick={() => setEditor(null)}>Fermer ×</button></div>
          <fieldset disabled={busy}>
            ...
            {section === 'carousel' && <label>Ordre d’affichage<input type="number" min="1" max="100" required value={editor.position} onChange={event => setEditor({ ...editor, position: Number(event.target.value) })} /><small>La plus petite valeur apparaît en premier.</small></label>}
          ...
          <div className="admin-editor-actions">{editor.id && <button className="admin-danger" type="button" disabled={busy} onClick={remove}>Supprimer</button>}<button type="button" disabled={busy} onClick={() => setEditor(null)}>Annuler</button><button className="admin-primary" disabled={busy}><AdminIcon name="check" />{busy ? 'Enregistrement…' : editor.status === 'published' ? 'Enregistrer et publier' : 'Enregistrer le brouillon'}</button></div>
        </form> : <>
          ...
          <section className="admin-content-list" aria-label="Contenus">
            <div className="admin-list-heading"><div><h2>Votre bibliothèque <span>{items.length}</span></h2><p>Retrouvez et gérez vos {sections[section].toLowerCase()}.</p></div><span className="admin-list-language"><AdminIcon name="globe" /> FR / EN</span></div>
